...
const normalizeProductName = (line: string): string | null => {
  ...
  const exactRule = PRODUCT_RULES.find((rule) => rule.keywords.some((keyword) => normalizedLine.includes(normalizeText(keyword))));
  if (exactRule) {
    const rejected = exactRule.reject?.some((word) => normalizedLine.includes(normalizeText(word))) ?? false;
    if (!rejected) return exactRule.normalizedName;
  }

  const fallback = cleanReceiptLine(line)
    .replace(/\b(desnatada|semidesnatada|entera|fresco|fresca|camp eros?|camperos?|rallado|rallada|natural|bio|eco|sin lactosa)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
