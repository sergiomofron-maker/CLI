...
import { Camera, Archive, Loader2, Plus, Trash2, Minus, X, FileText } from 'lucide-react';
import { parseReceiptProducts, ReceiptProductDraft } from '../services/receiptParser';
import { readReceiptImageText } from '../services/receiptOcrService';
...
const Inventory: React.FC<InventoryProps> = ({ userId }) => {
  ...
  const handleReceiptImageSelected = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const text = await readReceiptImageText(file, setOcrProgress);
      const detectedProducts = parseReceiptProducts(text);
    ...
    <div className="pb-24">
      ...
      {isScanningReceipt && (
        <div className="fixed bottom-36 left-1/2 z-30 w-56 -translate-x-1/2 rounded-xl bg-white px-4 py-3 text-center text-sm text-gray-700 shadow-lg border border-gray-100">
          Leyendo ticket... {ocrProgress > 0 ? `${ocrProgress}%` : ''}
        </div>
      )}
      ...
      {isReceiptReviewOpen && (
        <div className="fixed inset-0 z-40 flex items-end bg-black/40 sm:items-center sm:justify-center">
          <div className="max-h-[88vh] w-full overflow-y-auto rounded-t-2xl bg-white p-4 shadow-xl sm:max-w-lg sm:rounded-2xl">
            ...
            {receiptRawText && (
              <details className="mt-4 rounded-lg bg-gray-50 p-3 text-xs text-gray-500">
                <summary className="flex cursor-pointer items-center gap-2 font-medium text-gray-600">
                  <FileText size={14} /> Texto leído por OCR
                </summary>
                <pre className="mt-2 whitespace-pre-wrap break-words">{receiptRawText}</pre>
              </details>
            )}
